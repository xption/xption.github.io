...
export class RouteManager {
    ...
    routeMN(beginMN, endMN) {
        ...
        const findPath = (beginMN, endMN) => {
            ...
            const beginKey = calcTileIndex(beginMN.m, beginMN.n);
            
            const beginNode = new RouteNode(beginMN.m, beginMN.n);
            beginNode.gScore = 0;
            beginNode.fScore = heuristic(beginNode, endNode);
            openMap.set(beginKey, beginNode);

            // const endNode = new RouteNode(endMN.m, endMN.n);
            // endNode.gScore = Infinity;
            // endNode.fScore = Infinity;
            ...
            while (openMap.size > 0) {
                ...
                const currentKey = calcTileIndex(currentNode.m, currentNode.n);
                openMap.delete(currentKey);
                closeMap.set(currentKey, currentNode);

                // 判断到达终点
                if (currentNode.m === endMN.m && currentNode.n === endMN.n) {
                    console.log("到达终点: ", currentNode);
                ...
                for (const neighborOffset of neighborOffsetArray) {
                    ...
                    const neighborKey = calcTileIndex(neighborM, neighborN);
                    if (openMap.has(neighborKey) || closeMap.has(neighborKey)) {
                        continue;
                    }

                    neighborArray.push({ m: neighborM, n: neighborN });
                }

                // 遍历处理邻居
                for (const neighbor of neighborArray) {
                    const neighborKey = calcTileIndex(neighbor.m, neighbor.n);

                    const neighborNode = new RouteNode(neighbor.m, neighbor.n);
                    neighborNode.gScore = currentNode.gScore + 1;
                    neighborNode.fScore = heuristic(neighborNode, endMN);
        ...
        const reconstructPath = (cameFrom, current) => {
            const path = [current];
            while (cameFrom.has(calcTileIndex(current.m, current.n))) {
                current = cameFrom.get(calcTileIndex(current.m, current.n));
                path.unshift(current);
            }
            return path;
        };

        const calcTileIndex = (m, n) => {
            return m + n * 1000;
        };
